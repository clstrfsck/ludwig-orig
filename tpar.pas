...
{#if turbop}
                screen_msdos_string_message('Integer conversion failed.');
{#else}
                screen_message('Integer conversion failed.');
{#endif}
              reslen := 1;
              while (ord(result[i])<>0) do
                reslen := reslen+1;
{#endif}
              end
            ...
            if cmd = cmd_verify then
              ...
              if len = 0 then
                with cmd_attrib[cmd].tpar_info[this_tp] do
                  begin
{#if sun or turbop}
                  ch_copy(dflt_prompts[prompt_name],1,tpar_prom_len,
                          buffer,1,sizeof(buffer),' ');
                  verify_reply := screen_verify(buffer,tpar_prom_len);
{#else}
                  verify_reply := screen_verify(dflt_prompts[prompt_name],
                                                tpar_prom_len);
{#endif}
                  end
              else
...
{#else}
                  screen_getlinep(dflt_prompts[prompt_name],tpar_prom_len,
{#endif}
