...

{
...
#ifdef sun
    char        ch;
    struct      sgttyb  sgb_save, sgb;
    struct      timeval timeout;
    char        buffer[MAX_STRLEN], row_or_col[MAX_STRLEN];
    int         ret_len, j;
    int         error;
#endif
    ...
    *height = tt_termheight = 4;
	...
	error = timed_read(0, buffer, MAX_STRLEN, &timeout, 't', &ret_len);
	if (error >= 0) {
	    i = 0;
	    while ((buffer[i++] != ';') && (i < ret_len)) ;
	    if (i < ret_len) {
		j = 0;
		while ((buffer[i] != ';') && (i < ret_len))
		    row_or_col[j++] = buffer[i++];
		if (i < ret_len) {
		    row_or_col[j] = NUL;
		    if ((error = cvt_str_int(&tt_termheight, row_or_col)) > 0) {
			i += 1;
			j = 0;
			while ((buffer[i] != 't') && (i < ret_len))
			    row_or_col[j++] = buffer[i++];
			if (i <= ret_len) {
			    row_or_col[j] = NUL;
			    error = cvt_str_int(&tt_termwidth, row_or_col);
			} else
			    error = -1;
		    }
		} else
