...
{#if not turbop}
  ...
  procedure pattern_compound(    first       : nfa_state_range;
	...
	case pat_ch of
	  ...
	  pattern_lrange_delim:
	    ...
	    if not pattern_getch(parse_count,pat_ch,in_string) then
	      begin screen_message(msg_pat_no_matching_delim); goto 99; end;
		...
		begin screen_message(msg_pat_no_matching_delim); goto 99; end;
	      ...
	      begin screen_message(msg_pat_error_in_range); goto 99; end;
	    if pat_ch in ['0'..'9'] then
	      begin
	      parse_count := parse_count -1;
	...
	then { pat_ch not in syntax }
	  begin screen_message(msg_pat_illegal_symbol); goto 99; end;
	case pat_ch of
	  tpd_span,tpd_prompt:
	    begin
	    delimiter := pat_ch;
		...
		begin screen_message(msg_pat_error_in_span); goto 99; end;
	      end;
	    end;
	  ...
	  '{','}','<','>','^'    { positionals }:
	    ...
	    if pat_ch in [pattern_kstar,
	      ...
	      if not pattern_getch(parse_count,pat_ch,in_string) then
		begin screen_message(msg_pat_premature_pattern_end); goto 99; end;
		...
		pattern_mark:
		  ...
		  if pattern_getnumb(parse_count,aux,pat_ch,in_string) then
		    ...
		    if (aux = 0) or (aux > max_mark_number) then
		      begin screen_message(msg_pat_illegal_mark_number); goto 99; end;
		    with nfa_table[current_state] do
		      begin
		      epsilon_out := false;
		...
		if pat_ch in ['s','S','a','A','c','C','l','L','u','U',
		  ...
		  with nfa_table[current_state] do
		    ...
		    if pat_ch in ['<','>','{','}','^'] then
		      ...
		      if negate then
			begin screen_message(msg_pat_illegal_parameter); goto 99; end;
		      case pat_ch of
		  ...
		  begin screen_message(msg_pat_set_not_defined); goto 99; end;
	      end;
