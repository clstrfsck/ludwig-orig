...
#if vax && unix
#define filesys_create_open     filesyscreateopen
#define filesys_close           filesysclose
#define filesys_read            filesysread
#define filesys_rewind          filesysrewind
#define filesys_write           filesyswrite
#define filesys_parse           filesysparse
...
char *getwd(),*getenv(),*index(),*rindex(),*mktemp(),*strcpy();
extern int errno;

#ifdef TEST
#define screen_unix_message(string) fprintf(stderr,"%s\n",string)
#else
extern void screen_unix_message();
#endif
...
{
    char   tname[FILE_NAME_LEN],related[FILE_NAME_LEN+1];
    char   *p,*tmp,*nameptr;
...
# ifdef DEBUG
    ...
    if (fyle->zed != 'Z') {
	screen_unix_message("FILE and FILESYS definition of file_object disagree.");
	return 0;
    }
# endif
    fyle->l_counter = 0;
	...
	if (!ordinary_open) { /* really executing a command */
	    if (pipe(fd) == -1) {
		screen_unix_message("Cannot create pipe");
		return 0;
	    }
	    if ((pid = fork()) == -1) {
		screen_unix_message("Failed to fork");
		return 0;
	    } else if (pid == 0) {
		close(fd[0]);
		...
		sprintf(msg_string,"Error in filename (%s)",fyle->fnm);
		screen_unix_message(msg_string);
		return 0;
	    }
	...
	if (exists) {
	    ...
	    if (fyle->create) {
		sprintf(msg_string,"File (%s) already exists",fyle->fnm);
		screen_unix_message(msg_string);
		return 0;
	    }
	    /* check that the file we may overwrite is not a directory */
	    if ((stat_buffer.st_mode & S_IFMT) == S_IFDIR) {
		sprintf(msg_string,"File (%s) is a directory",fyle->fnm);
		screen_unix_message(msg_string);
		return 0;
	    }
	    /* check that we can write over the current version */
	    if (access(fyle->fnm,W_OK) == -1) {
		sprintf(msg_string,"Write access to file (%s) is denied",fyle->fnm);
		screen_unix_message(msg_string);
		return 0;
	    }
	    fyle->mode = stat_buffer.st_mode & 07777;
	...
	uniq = 0;
	sprintf(fyle->tnm,"%s-lw",fyle->fnm);
	while (access(fyle->tnm,F_OK) == 0)
	    sprintf(fyle->tnm,"%s-lw%d",fyle->fnm,++uniq);
	if ((fyle->fd = open(fyle->tnm, O_WRONLY | O_CREAT, 0600)) < 0) {
	    sprintf(msg_string,"Error opening (%s) as output",fyle->tnm);
	    screen_unix_message(msg_string);
	    return 0;          /* fail,    return false */
	}
    }
...
*/
filesys_close(fyle, delete, msgs)
FILE_PTR fyle;
BOOLEAN  delete, msgs;

/* Closes a file, described by fileptr fyle. The msgs parameter indicates
 * whether we want to be told about what is going on. While delete is a
 * boolean to indicate whether to remove the file if an output file.
...

{
	...
	if (close(fyle->fd) < 0)
	    return 0;          /* fail,    return false */
	if (msgs) {
	    sprintf(msg_string,"File %s closed (%d line%s read).",
		    fyle->fnm,fyle->l_counter,(fyle->l_counter == 1)?"":"s");
	    screen_unix_message(msg_string);
	}
	return 1;              /* succeed, return true */
    }
	...
	if (!unlink(fyle->tnm)) {
	    if (msgs) {
		sprintf(msg_string,"Output file %s deleted.",fyle->tnm);
		screen_unix_message(msg_string);
	    }
	    return 1;          /* succeed, return true  */
	} else
	...
	if (stat_buffer.st_mtime != fyle->previous_file_id) {
	    sprintf(msg_string,"%s was modified by another process",fyle->fnm);
	    screen_unix_message(msg_string);
	}
    strcpy(dir,fyle->fnm);
    ...
    *s = '\0';
	...
	for (dp = readdir(dirp); dp != NULL;dp = readdir(dirp))
		...
		if (i < fyle->versions-1) {
			...
			sprintf(temp,bname,versions[entries-1]);
			sprintf(spec,"%s/%s",dir,temp);
			unlink(spec);
		    } else
			entries++;
		    ...
		    sprintf(spec,"%s/%s",dir,dp->d_name);
		    unlink(spec);
		}
	    }
	max_vnum = versions[0];
	...
	sprintf(msg_string,"fylesysclose: cannot rename %s to %s\n",
		fyle->tnm,fyle->fnm);
	screen_unix_message(msg_string);
	return 0;          /* fail,    return false */
    } else {
	if (msgs) {
	    sprintf(msg_string,"File %s created (%d line%s written).",
		    fyle->fnm,fyle->l_counter,(fyle->l_counter == 1)?"":"s");
	    screen_unix_message(msg_string);
	}
	/*
	 ...
	 */
	if (strcmp(dir,"/tmp") != 0 &&
	    strcmp(dir,"/usr/tmp") != 0 && fyle->memory != 0) {
	    if ((envfile = fopen(fyle->memory,"w")) == NULL)
		return 1;
	    fprintf(envfile,"%s\n",fyle->fnm);
	    fclose(envfile);
	}
...
/* Attempts to write bufsiz characters from buffer to the file described by fyle.
 * Returns true (1) on success, false (0) on failure.
 */
...

{
 ...
 [file [file]]";
    ...
    long    c,argc,errors,versions,space,ch_length();
    ...
    command_line[ch_length(command_line CONF,FILE_NAME_LEN)] = '\0';
    argv[0] = "Ludwig";
    argc = 1;
	...
	sprintf(def_init,"%s/.ludwigrc",getenv("HOME"));
	initialize = def_init;
	sprintf(def_memory,"%s/.lud_memory",getenv("HOME"));
	memory = def_memory;
    } else {
	initialize = NULL;
	memory = NULL;
    ...
    errno = 0;
    optind = 1;
    while ((c = getopt(argc,argv,"cri:Is:m:MtTb:B:oOu")) != EOF)
	switch (c) {
	case 'c' :
	    if (read_only_flag)
		errors++;
    ...
    if (parse == PARSE_COMMAND) {
	if (initialize)
	    ch_copy(initialize CONF,1,strlen(initialize),
		   file_data->initial CONF,1,MAX_STRLEN,' ');
	else
	    ch_fill(file_data->initial CONF,1,MAX_STRLEN,' ');
	...
	if (files >= 2) {
	    screen_unix_message("More than two files specified");
	    return 0;
	}
	...
	if (   parse == PARSE_INPUT || parse == PARSE_OUTPUT
	    ...
	    screen_unix_message("Only one file name can be specified");
	    return 0;
	}
    }
    switch (parse) {
	...
	case PARSE_EDIT :
		...
		if ((envfile = fopen(memory,"r")) != NULL) {
		    fscanf(envfile,"%s\n",(*input)->fnm);
		    (*input)->fns = strlen((*input)->fnm);
		    fclose(envfile);
		    check_input = 1;
		} else {
		    sprintf(msg_string,"Error opening memory file (%s)",memory);
		    screen_unix_message(msg_string);
		    if (parse == PARSE_COMMAND) {
		      (*input)->fns = 0;
		      return 1;
		...
		if (filesys_create_open(*input,NULL,TRUE) == 0) {
		    sprintf(msg_string,"Error opening (%s) as input",
				       (*input)->fnm);
		    screen_unix_message(msg_string);
		    return 0;
		}
	...
	case PARSE_INPUT :
		...
		(*input)->fns = strlen(file[0]);
		strcpy((*input)->fnm,file[0]);
	    } else if (memory != NULL && (envfile = fopen(memory,"r")) != NULL) {
		fscanf(envfile,"%s\n",(*input)->fnm);
		(*input)->fns = strlen((*input)->fnm);
		fclose(envfile);
	    } else {
		...
		sprintf(msg_string,"Error opening (%s) as input",(*input)->fnm);
		screen_unix_message(msg_string);
		return 0;
	    }
	    (*input)->valid = 1;
...
static filesys_expand_file_name(fnm,fns)
char *fnm;
long *fns;
