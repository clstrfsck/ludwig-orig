...
{#if turbop}
{##procedure screen_msdos_message #<(}
{##        var     message)#>;}
{##}
{##  var}
{##    msg : msg_str;}
...
{##  start := 1;}
{##  len := max_strlen;}
{##  while str_object(message)[len] = ' ' do}
{##    dec(len);}
{##  while len > msg_str_len do}
{##    begin}
{##    ch_copy(message, start, msg_str_len, msg, 1, msg_str_len, ' ');}
{##    screen_message(msg);}
{##    start := start + msg_str_len;}
{##    len := len - msg_str_len;}
{##    end;}
{##  ch_copy(message, start, len, msg, 1, msg_str_len, ' ');}
{##  screen_message(msg);}
{##end; #<screen_msdos_message#>}
{##}
{##procedure screen_msdos_string_message#<(message:string)#>;}
{##}
{##var}
{##buffer : msg_str;}
{##begin}
...
{##screen_message(buffer);}
{##end; #<screen_msdos_string_message#>}
{#endif}
...
{##    length := length+1;}
{##  while length > msg_str_len do}
{##    begin}
{##    ch_copy(message, start, msg_str_len, msg, 1, msg_str_len, ' ');}
{##    screen_message(msg);}
{##    start := start + msg_str_len;}
{##    length := length - msg_str_len;}
{##    end;}
{##  ch_copy(message, start, length, msg, 1, msg_str_len, ' ');}
{##  screen_message(msg);}
...
function screen_vms_message {(
  ...
  while length > msg_str_len do
    begin
    msg := substr(message, start, msg_str_len);
    screen_message(msg);
    start := start + msg_str_len;
    length := length - msg_str_len;
    end;
...
{#if debug}
  if bot_line^.scr_row_nr <> row_nr then
    begin screen_message(dbg_wrong_row_nr); goto 99; end;                     {Finished.             }
{#endif}
  99:
  end; {screen_scroll}
...
{#if sun}
{##  i := 1;}
{##  while (i <= prompt_len) and (i < msg_str_len) do}
{##    begin}
{##    prompt_msg[i] := prompt[i];}
{##    i := i + 1;}
{##    end;}
{##  for i := prompt_len + 1 to msg_str_len do}
{##    prompt_msg[i] := ' ';}
{#else}
  for i := prompt_len + 1 to p_len do
    prompt[i] := ' ';
{#endif}
  with current_frame^,dot^ do
    begin
      ...
      case ludwig_mode of
        ludwig_screen:
          ...
          if use_prompt then
            begin
{#if turbop}
{##            screen_msdos_message(prompt);}
{#elseif sun}
{##            screen_message(prompt_msg);}
{#else}
...
{#else}
{#if sun}
{##            msg := 'Reply Y(es),N(o),A(lways),Q(uit),M(ore)';}
{##            screen_message(msg)}
{#else}
