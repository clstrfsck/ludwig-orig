...
function pattern_dfa_convert {(
	...
	begin
	if not push_stack(state_elt_ptr^.state_elt) then
	  begin
	...
	with nfa_table[aux_state] do
	  ...
	  if epsilon_out then
	    ...
	    if first_out <> pattern_null then
		 ...
		 if not push_stack(first_out) then
		   begin
		   epsilon_closures := false;
		   goto 99;
		   end;
		...
		if not push_stack(second_out) then
		  begin
		  epsilon_closures := false;
		  goto 99;
		  end;
	...
	transition_set : nfa_attribute_type;
      ...
      if not epsilon_closures(transition_set,transition_set) then
	begin
    ...
    function pattern_new_dfa(    equivalent_set : nfa_attribute_type;
			     var state_count : dfa_state_range):boolean;
      var
	i       : nfa_state_range;
	aux_elt : state_elt_ptr_type;
	...
	with dfa_table_pointer^ do
	  ...
	  for i := 0 to states_used do
	    if state_head = dfa_table[i]
		 .nfa_attributes.equiv_set then
	      begin
	      dfa_search := true;
	      position := i;
	      goto 1;
	...
	if not pattern_new_dfa(transfer_state,position) then    { create new DFA state }
	  begin
