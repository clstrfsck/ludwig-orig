...
!       Inserted code to account for the new option, 'opt_save_frame'
!       into show_options, frame_options and set_options.
!--}
...
{}procedure show_options;
	...
	screen_write_str(0,'Off',3);
      screen_writeln;
      screen_write_str(4,'Save Frame            F       ',30);
...
{}function set_options (var request     : tpar_object;
	...
	case ch of
	  ...
	  'N' : if seton then
		  ...
		  options := options - [opt_new_line];
	  'F' : if seton then
		  options := options + [opt_save_frame]
		else
		  options := options - [opt_save_frame];
	end{case};
	set_opt := true;
...
function frame_parameter {(
  ...
  repeat
    ...
    screen_writeln;
    screen_writeln_clel;
    screen_getlinep('  New Values: ',14,request.str,request.len,1,1);
    if request.len > 0 then
      begin
