...
{$I code.h}
implementation
uses ch, exec, frame, line, mark, screen, tpar, vduibm;
{#elseif vms}
%include 'const.inc/nolist'
%include 'type.inc/nolist'
%include 'var.inc/nolist'

%include 'code.fwd/nolist'
...
{#endif}
  ...
  function generate (irep:leadparam; icnt:integer; iop:commands;
    ...
    if code_base + pc > max_code then
      begin
      status := msg_compiler_code_overflow;
      generate := false;
      goto 99;
      end;
      ...
      if key in [ord('0')..ord('9'), ord('+'), ord('-'), ord('>'), ord('.'),
        ...
        case chr(key) of
          ...
          '@'     : begin
                    ...
                    repsym := marker;
                    if getcount and
                       ((repcount = 0) or (repcount > max_mark_number)) then
                      begin
{#if turbop}
                      fillchar(msg_buffer[1] , sizeof(msg_buffer), ' ');
    ...
    function scan_exit_handler:boolean;
      ...
      if key = ord('[') then
        ...
        while (key <> ord(':')) and (key <> ord(']')) do
          if not scan_command(full_scan) then
            begin    {Construct exit part. }
            scan_exit_handler := false;
            goto 99;
            end;
        if key = ord(':') then
          begin
          if not generate(none,0,cmd_pcjump,nil,0,nil) then
            begin    {Jump over fail handler.}
            scan_exit_handler := false;
            goto 99;
            end;
          pc4 := pc;
...
{#endif}
      ...
      if lookup[key].tpar = nil then
        if cmd_attrib[command].tpcount <> 0 then
          if full_scan then
            begin
            if not scan_trailing_param then
              begin
              scan_simple_command := false;
              goto 99;
              end;
      ...
      if not generate(repsym,repcount,command,tparam,0,lookup_code) then
        begin
        scan_simple_command := false;
        goto 99;
        end;
...
{#endif}
      ...
      if not generate(none,0,cmd_exitto,nil,0,nil) then
        begin
        scan_compound_command := false;
        goto 99;
        end;
      pc2 := pc;
      if not generate(none,0,cmd_failto,nil,0,nil) then
        begin
        scan_compound_command := false;
        goto 99;
        end;
      ...
      if repsym <> pindef then
        if not generate(none,repcount,cmd_iterate,nil,0,nil) then
          begin
          scan_compound_command := false;
          goto 99;
          end;
      ...
      repeat
        if not scan_command(true) then
          begin
          scan_compound_command := false;
          goto 99;
          end;
      until key = ord(')');
      if not generate(none,0,cmd_pcjump,nil,pc3,nil) then
        begin
        scan_compound_command := false;
        goto 99;
        end;
    ...
    begin {scan_command}
    scan_command := true;
    if not scan_leading_param then
      begin
      scan_command := false;
      goto 99;
      end;
...
{#endif}
    ...
    if key = ord('(') then
      begin
      if not scan_compound_command then
        begin
        scan_command := false;
        goto 99;
        end;
    ...
    if command <> cmd_noop then
      begin
      if not scan_simple_command then
        begin
        scan_command := false;
        goto 99;
        end;
...
{#endif}
    ...
    if full_scan then
      scan_command := scan_exit_handler;
   99:
    end; {scan_command}
