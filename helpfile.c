...

{
    ...
    char *help,*getenv(),buffer[ENTRY_SIZE+1];
...

{
	...
	if (strcmp(string,"exit") == 0)
	    exit(0);
	if (helpfileread(string,strlen(string),buffer,ENTRY_SIZE,&buflen)) {
	    buffer[KEY_SIZE+buflen] = '\0';
	    fputs(&buffer[KEY_SIZE],stderr);
	    fputc('\n',stderr);
	    while (helpfilenext(buffer,ENTRY_SIZE,&buflen)) {
		buffer[KEY_SIZE+buflen] = '\0';
		fputs(&buffer[KEY_SIZE],stderr);
		fputc('\n',stderr);
	    }
